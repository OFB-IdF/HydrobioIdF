...
limites_ibd  <- read.csv2("EBio_CE_2015_params_IBD.csv",
                          stringsAsFactors = FALSE)
limites_ibmr <- read.csv2("EBio_CE_2015_params_IBMR.csv",
                          stringsAsFactors = FALSE)
limites_ibg  <- read.csv2("EBio_CE_2015_params_IBG-DCE.csv", 
                          stringsAsFactors = FALSE)
limites_mgce <- read.csv2("EBio_CE_2015_params_MGCE.csv",
                          stringsAsFactors = FALSE)
limites_ipr  <- read.csv2("EBio_CE_2015_params_IPR.csv",
                          stringsAsFactors = FALSE)
...
funClass <- function(valeurs, limites) {
    ...
    if (nrow(valeurs) == 0) {
        ...
        if (unique(valeurs$CODE_PAR) %in% c(5856, 2928, 5910, 6951)) {
...
table_ibd <- funMoyenne(
    valeurs    = data_ibd,
    dateFormat = "%d/%m/%Y",
    sites      = data_station
)

table_ibmr <- funMoyenne(
    valeurs    = data_ibmr,
    dateFormat = "%d/%m/%Y",
    sites      = data_station
)

table_mib <- funMoyenne(
    valeurs    = data_mib,
    dateFormat = "%d/%m/%Y",
    sites      = data_station
)

table_ipr <- funMoyenne(
    valeurs    = data_ipr,
    dateFormat = "%d/%m/%Y",
    sites      = data_station
) 
...
# Calcul des EQR
if (nrow(table_mib) > 0) {
  if (unique(table_mib$CODE_PAR) == 5910) {
    limites_mib <- limites_ibg
} else {
...
eqr_ipr <- funClass(
    valeurs = left_join(eqr_ipr, alt, by = "CODE_STATION"),
    limites = limites_ipr
)

eqr <- bind_rows(eqr_ibd, eqr_ibmr, eqr_mib, eqr_ipr) %>%
    mutate(LIB_PAR = indicateurs$LIB_PAR[match(
        CODE_PAR,
        indicateurs$CODE_PAR)] %>%
            as.character())                                       %>%
